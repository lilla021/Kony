...
moderateBehavior::moderateBehavior()
{
}

void moderateBehavior::setFirstPosition(Map* myMap, Player* p)
{
...
void moderateBehavior::chooseMonster(Map* myMap, Player* p)
{
	NotifyPhase(p->getPlayerId(), "\nCHOOSE MONSTER");
...
void moderateBehavior::rollDice(Player* p)
{
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	NotifyPhase(p->getPlayerId(), "\nROLL DICE");
	srand(time_t(0));
	...
	while ((p->keepTracker != 6) && (p->rollCounter <= 4))
	{
		...
		for (size_t i = 0; i < p->blackDice.size(); i++)
		{
			...
			{
				if ((p->blackDice[i].getResult() == "Heal") || (p->blackDice[i].getResult() == "Energy"))
				{
					input == 'y';
					cout << endl << "Y";
			...
			else if ((p->blackDice[i].getResult() == "Energy"))
			{
				input == 'y';
				cout << endl << "Y";
				cout << endl << "Keep - Dice " << i + 1 << ": " << p->blackDice[i].getResult() << endl;
...
void moderateBehavior::resolveDice(Map* myMap, Player* p)
{
	NotifyPhase(p->getPlayerId(), "\nRESOLVE DICE");
...
void moderateBehavior::move(Map* myMap, Player* p)
{
	NotifyPhase(p->getPlayerId(), "\nMOVE");
	char input;
...
void moderateBehavior::buyCards(Map* myMap, Player* p)
{
	char input;
	int cardNumber;
	do
...
void moderateBehavior::displayStats(Player* p) {
	NotifyStats();
}
void moderateBehavior::displayDice(Player* p) {
	NotifyDice();
}
void moderateBehavior::displayCards() {
	NotifyCard();
}
moderateBehavior::~moderateBehavior()
{
